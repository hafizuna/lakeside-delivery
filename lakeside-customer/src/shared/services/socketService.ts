...
class SocketService {
  ...
  private orderUpdateListeners: Array<(data: OrderStatusUpdateData) => void> =
    [];
  private orderCancellationListeners: Array<
    (data: OrderCancellationData) => void
  > = [];
  ...
  public async connect(): Promise<boolean> {
    try {
      ...
      if (!this.isAppActive || !this.isNetworkConnected) {
        console.log(
          "🔌 Skipping socket connection - app not active or network unavailable"
        );
        return false;
      }
      ...
      const socketUrl = "http://192.168.1.2:3001";
        ...
        this.socket.once("connect", () => {
          ...
          this.authenticate()
            .then((success) => {
              if (success) {
                console.log(
                  "🎉 Socket connection and authentication complete!"
                );
                resolve(true);
              } else {
  ...
  private async authenticate(): Promise<boolean> {
    ...
    return new Promise((resolve, reject) => {
      ...
      console.log(
        "📤 Emitting authenticate event with token length:",
  ...
  private setupSocketListeners(): void {
    ...
    this.socket.on("error", (error: any) => {
      ...
      this.errorListeners.forEach((listener) =>
        listener(error.message || error, error.code)
      );
    });
  ...
  public onOrderUpdate(
    listener: (data: OrderStatusUpdateData) => void
  ): () => void {
    this.orderUpdateListeners.push(listener);
    return () => {
      this.orderUpdateListeners = this.orderUpdateListeners.filter(
        (l) => l !== listener
      );
    };
  }

  public onOrderCancellation(
    listener: (data: OrderCancellationData) => void
  ): () => void {
    this.orderCancellationListeners.push(listener);
    return () => {
      this.orderCancellationListeners = this.orderCancellationListeners.filter(
        (l) => l !== listener
      );
    };
  }
  ...
  public onNotification(
    listener: (data: SocketNotificationData) => void
  ): () => void {
