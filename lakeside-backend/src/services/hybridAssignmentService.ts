import { PrismaClient, DriverAssignmentStatus, OrderStatus } from '@prisma/client';
import {
  AvailableDriver,
  CreateOffersRequest,
  AssignmentCreationResult,
  AcceptOfferRequest,
  AssignmentAcceptanceResult,
  UpdateDriverStateRequest,
  DriverHeartbeatRequest,
  WaveEscalationConfig,
  AssignmentAnalytics
...
class HybridAssignmentService {
   ...
   */
  async updateDriverState(driverId: number, updates: UpdateDriverStateRequest) {
    try {
      const now = new Date();
   ...
   */
  async processDriverHeartbeat(request: DriverHeartbeatRequest) {
    try {
      const now = new Date();
   ...
   */
  async findAvailableDrivers(orderId: number, maxDrivers: number = 5): Promise<AvailableDriver[]> {
    try {
      ...
      const availableDrivers: AvailableDriver[] = drivers
        .filter(driver => {
          const state = driver.driverState;
   ...
   */
  async createOffers(request: CreateOffersRequest): Promise<AssignmentCreationResult> {
    try {
      ...
      const assignmentPromises = driverIds.map(driverId =>
        prisma.driverAssignment.create({
          data: {
            orderId,
            driverId,
            status: 'OFFERED' as DriverAssignmentStatus,
            wave,
            offeredAt: now,
            expiresAt,
          }
   ...
   */
  async acceptOffer(request: AcceptOfferRequest): Promise<AssignmentAcceptanceResult> {
    ...
    try {
      ...
      const result = await prisma.$transaction(async (tx) => {
        ...
        const acceptedAssignment = await tx.driverAssignment.update({
          where: { id: assignmentId },
          data: {
            status: 'ACCEPTED' as DriverAssignmentStatus,
            respondedAt: now,
            acceptedAt: now,
          }
        });
        ...
        const expiredCount = await tx.driverAssignment.updateMany({
          ...
          data: {
            status: 'EXPIRED' as DriverAssignmentStatus,
            respondedAt: now,
          }
        });
   ...
   */
  async completeAssignment(orderId: number) {
    try {
      const now = new Date();
      ...
      await prisma.$transaction(async (tx) => {
        ...
        await tx.driverAssignment.update({
          ...
          data: {
            status: 'COMPLETED' as DriverAssignmentStatus,
            updatedAt: now,
          }
        });
   ...
   */
  async getDriverOffers(driverId: number) {
    try {
      const now = new Date();
   ...
   */
  private getWaveEscalationConfig(orderId: number, currentWave: number) {
    ...
    const waveKey = `wave${Math.min(currentWave, 4)}` as keyof typeof baseConfig;
   ...
   */
  async findAvailableDriversForWave(orderId: number, wave: number, maxDrivers: number = 5): Promise<AvailableDriver[]> {
    try {
      const order = await prisma.order.findUnique({
        where: { id: orderId },
        include: { restaurant: true }
      ...
      const availableDrivers: AvailableDriver[] = scoredDrivers.map(({ driver }) => ({
        driverId: driver.id,
        name: driver.user.name,
        currentLat: driver.currentLat?.toNumber(),
        currentLng: driver.currentLng?.toNumber(),
   ...
   */
  private calculateDriverScore(driver: any, order: any, wave: number): number {
    let score = 0;
   ...
   */
  async createWaveAssignmentOffers(orderId: number, wave: number = 1) {
    try {
      const waveConfig = this.getWaveEscalationConfig(orderId, wave);
      
      // Find drivers using wave-enhanced selection
      const availableDrivers = await this.findAvailableDriversForWave(
        orderId, 
        wave, 
        waveConfig.driverCount
      );
   ...
   */
  async getWaveEscalationStatus(orderId: number) {
    try {
   ...
   */
  async declineOffer(assignmentId: string, driverId: number, reason?: string) {
    try {
      const now = new Date();
      ...
      const declinedAssignment = await prisma.driverAssignment.update({
        where: { id: assignmentId },
        data: {
          status: 'DECLINED' as DriverAssignmentStatus,
          respondedAt: now,
   ...
   */
  async getDeclineReasonsAnalytics(timeframeHours: number = 24) {
    try {
      ...
      const waveDeclines: Record<number, number> = {};
...
export default new HybridAssignmentService();