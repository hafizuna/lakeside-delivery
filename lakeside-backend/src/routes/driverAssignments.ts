...
import hybridAssignmentService from '../services/hybridAssignmentService';
import { 
  AcceptOfferRequest,
  UpdateDriverStateRequest,
...
router.post('/state', authenticateToken, async (req, res) => {
  try {
    ...
    const updates: UpdateDriverStateRequest = req.body;
    const result = await hybridAssignmentService.updateDriverState(driverId, updates);
...
router.post('/heartbeat', authenticateToken, async (req, res) => {
  try {
    ...
    const heartbeatData: DriverHeartbeatRequest = {
      driverId,
      ...req.body
    };

    const result = await hybridAssignmentService.processDriverHeartbeat(heartbeatData);
...
router.get('/offers', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await hybridAssignmentService.getDriverOffers(driverId);
 ...
 */
router.post('/offers/:assignmentId/accept', authenticateToken, async (req, res) => {
  try {
    const driverId = req.user?.id;
    ...
    const acceptRequest: AcceptOfferRequest = {
      assignmentId,
      driverId
    };

    const result = await hybridAssignmentService.acceptOffer(acceptRequest);
 ...
 */
router.post('/offers/:assignmentId/decline', authenticateToken, async (req, res) => {
  try {
    const driverId = req.user?.id;
    const assignmentId = req.params.assignmentId;
    ...
    const result = await hybridAssignmentService.declineOffer(assignmentId, driverId, reason);
...
router.post('/cleanup-expired', authenticateToken, async (req, res) => {
  try {
    const result = await hybridAssignmentService.cleanupExpiredAssignments();
 ...
 */
router.get('/orders/:orderId/waves', authenticateToken, async (req, res) => {
  try {
    const orderId = parseInt(req.params.orderId);
    const waveStatus = await hybridAssignmentService.getWaveEscalationStatus(orderId);
 ...
 */
router.post('/orders/:orderId/escalate', authenticateToken, async (req, res) => {
  try {
    const orderId = parseInt(req.params.orderId);
    const { wave } = req.body;

    const result = await hybridAssignmentService.createWaveAssignmentOffers(orderId, wave || undefined);
    ...
    res.json({
      ...
      data: {
        ...
        availableDrivers: (result as any).availableDrivers || 0
      },
      timestamp: new Date()
    });
  } catch (error) {
 ...
 */
router.get('/analytics/declines', authenticateToken, async (req, res) => {
  try {
    const timeframeHours = parseInt(req.query.hours as string) || 24;
    const analytics = await hybridAssignmentService.getDeclineReasonsAnalytics(timeframeHours);
 ...
 */
router.get('/system/maintenance', authenticateToken, async (req, res) => {
  try {
 ...
 */
router.post('/system/maintenance/run', authenticateToken, async (req, res) => {
  try {
    const result = await maintenanceService.runManualMaintenance();
 ...
 */
router.post('/system/emergency-cleanup', authenticateToken, async (req, res) => {
  try {
...
router.get('/dashboard', authenticateToken, async (req, res) => {
  try {
    ...
    const dashboard = await driverDashboardService.getDriverDashboard(driverId, timeframe);
 ...
 */
router.get('/dashboard/earnings', authenticateToken, async (req, res) => {
  try {
    const driverId = req.user?.id;
    if (!driverId) {
      return res.status(401).json({ success: false, message: 'Driver not authenticated' });
    ...
    const earningsReport = await driverDashboardService.getEarningsReport(driverId, timeframe);
