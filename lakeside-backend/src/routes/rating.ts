 ...
 * Rate a restaurant
 */
router.post('/restaurant', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
...
router.post('/order', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const orderAvgRatingResult = await prisma.rating.aggregate({
      where: {
        orderId: parseInt(orderId),
        ratingType: 'ORDER'
      },
    ...
    res.status(200).json({
      ...
      data: {
        ...
        pagination: {
          ...
          totalPages: Math.ceil(totalRatings / parseInt(limit as string)),
          totalRatings,
 ...
 */
router.get('/check/:type/:targetId', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
 ...
 * Rate a driver
 */
router.post('/driver', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
