...
export const validateUserTypeQuery = [
  query('userType')
    .optional()
...
export const checkWalletOwnership = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
...
export const checkTopUpLimits = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
...
export const checkWithdrawalEligibility = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const userId = req.user?.id;
    const userRole = req.user?.role;
