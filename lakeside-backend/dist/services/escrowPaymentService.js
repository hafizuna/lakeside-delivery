...
class EscrowPaymentServiceImpl {
    /**
     ...
     */
    async processEscrowPayment(orderId) {
        try {
            const order = await prisma.order.findUnique({
                where: { id: orderId }
            });
     ...
     */
    async cancelOrderWithRefund(orderId, reason) {
        try {
            ...
            const result = await prisma.$transaction(async (tx) => {
                ...
                const cancelledOrder = await tx.order.update({
                    ...
                    data: {
                        ...
                        platformEarnings: new client_1.Prisma.Decimal(0),
                        restaurantCommission: new client_1.Prisma.Decimal(0),
                        deliveryCommission: new client_1.Prisma.Decimal(0)
                    }
                });
                // Process refund based on payment status
                if (order.paymentStatus === 'ESCROWED' && order.paymentMethod === 'WALLET') {
                    // For escrowed wallet payments, add refund to customer wallet
                    const refund = await walletService_1.default.addCustomerRefund(order.customerId, totalAmount, orderId, `Order cancelled: ${reason}`);
                    if (!refund.success) {
     ...
     */
    async releaseEscrowOnDelivery(orderId, driverId) {
        try {
            const order = await prisma.order.findUnique({
                where: { id: orderId },
                select: {
     ...
     */
    async checkRestaurantTimeout(orderId) {
        try {
            const order = await prisma.order.findUnique({
                where: { id: orderId }
            });
            ...
            const orderAge = now.getTime() - order.createdAt.getTime();
            const fifteenMinutes = 15 * 60 * 1000; // 15 minutes in milliseconds
            const hasTimedOut = (orderAge > fifteenMinutes &&
     ...
     */
    async processTimeoutRefund(orderId) {
        try {
...
exports.default = escrowPaymentService;
//# sourceMappingURL=escrowPaymentService.js.map