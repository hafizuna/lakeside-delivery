...
class WalletService {
    ...
    async requestCustomerTopUp(customerId, amount, screenshotUrl) {
        try {
            if (amount <= 0) {
    ...
    async processCustomerPayment(customerId, amount, orderId) {
        try {
    ...
    async createDriverWallet(driverId) {
        try {
    ...
    async addDriverEarning(driverId, amount, orderId) {
        try {
            const result = await prisma.$transaction(async (tx) => {
    ...
    async requestDriverWithdrawal(driverId, amount) {
        try {
    ...
    async addCustomerRefund(customerId, amount, orderId, reason) {
        try {
            const result = await prisma.$transaction(async (tx) => {
    ...
    async getTransactionHistory(userId, userType = 'customer', page = 1, limit = 20) {
        try {
    ...
    async approveTransaction(transactionId, adminId, adminNotes) {
        try {
    ...
    async rejectTransaction(transactionId, adminId, adminNotes) {
        try {
    ...
    // Utility Methods
    async getWalletStats(userType = 'all') {
        try {
            const stats = {};
    ...
    async refundCustomer(customerId, amount, orderId, adminId) {
        try {
            const result = await prisma.$transaction(async (tx) => {
                // Update customer wallet
                const wallet = await tx.customerWallet.upsert({
    ...
    async checkSufficientBalance(customerId, amount) {
        try {
            const wallet = await prisma.customerWallet.findUnique({
                where: { customerId }
            });
