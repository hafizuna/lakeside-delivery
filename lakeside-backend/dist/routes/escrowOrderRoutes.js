...
router.get('/:id/can-cancel', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const canCancelResult = await escrowPaymentService_1.default.canCancelOrder(orderId);
        res.json({
            success: true,
...
router.post('/:id/process-escrow', auth_1.authenticateToken, async (req, res) => {
    try {
        const orderId = parseInt(req.params.id);
        // This endpoint can be called by system cron job or manually for testing
        const result = await escrowPaymentService_1.default.processEscrowPayment(orderId);
        if (!result.success) {
...
router.post('/:id/cancel', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const result = await escrowPaymentService_1.default.cancelOrderWithRefund(orderId, cancelReason);
        if (!result.success) {
            return res.status(400).json(result);
        }
        res.json({
...
router.post('/:id/accept', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const timeoutCheck = await escrowPaymentService_1.default.checkRestaurantTimeout(orderId);
...
router.post('/:id/deliver', auth_1.authenticateToken, async (req, res) => {
    try {
        const orderId = parseInt(req.params.id);
        ...
        const result = await escrowPaymentService_1.default.releaseEscrowOnDelivery(orderId, driverId);
        if (!result.success) {
            return res.status(400).json(result);
        }
        res.json({
...
router.get('/:id/timeout-check', auth_1.authenticateToken, async (req, res) => {
    try {
        const orderId = parseInt(req.params.id);
        const timeoutCheck = await escrowPaymentService_1.default.checkRestaurantTimeout(orderId);
...
router.post('/:id/timeout-refund', auth_1.authenticateToken, async (req, res) => {
    try {
        const orderId = parseInt(req.params.id);
        const result = await escrowPaymentService_1.default.processTimeoutRefund(orderId);
        if (!result.success) {
            return res.status(400).json(result);
        }
        res.json({
...
router.get('/:id/cancellation-info', auth_1.authenticateToken, async (req, res) => {
    try {
        const orderId = parseInt(req.params.id);
        const userId = req.user?.id;
        if (!userId) {
...
//# sourceMappingURL=escrowOrderRoutes.js.map