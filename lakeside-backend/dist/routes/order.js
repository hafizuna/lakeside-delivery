...
router.post('/', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        if (paymentMethod.toUpperCase() === 'WALLET') {
            ...
            const balanceCheck = await walletService_1.default.checkSufficientBalance(userId, totalAmount);
...
router.patch('/:id/deliver', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            await walletService_1.default.addDriverEarning(driverId, driverEarning, orderId);
