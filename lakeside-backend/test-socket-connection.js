...
async function runConnectionTests() {
  ...
  if (successfulConnections.length > 0) {
    ...
    if (successfulConnections.find((r) => r.url.includes("192.168.1.5"))) {
      ...
      log(
        "⚠️  Network IP not accessible - React Native may have issues",
        "yellow"
      );
      log(
        "💡 Try updating React Native client to use localhost or check network settings",
        "yellow"
      );
    }
  } else {
    ...
    log(
      "❌ Socket.IO server may not be running or configured incorrectly",
      "red"
    );
  }
}
