...
class ApiService {
  ...
  private async request<T>(
    ...
    console.log('API Request Debug:', {
      ...
      headers: { ...headers, Authorization: headers.Authorization ? 'Bearer ***' : 'No auth' }
    });
  ...
  // Authentication
  async login(phone: string, password: string): Promise<ApiResponse<{ token: string; restaurant: any }>> {
    const response = await this.request<{ token: string; restaurant: any }>('/restaurant/auth/login', {
      method: 'POST',
  ...
  async toggleMenuItemAvailability(id: number, isAvailable: boolean): Promise<ApiResponse<any>> {
  ...
  // Orders Management
  async getOrders(status?: string, limit?: number, offset?: number): Promise<ApiResponse<any[]>> {
